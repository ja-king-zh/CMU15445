...
BufferPoolManager::BufferPoolManager(size_t num_frames, DiskManager *disk_manager, size_t k_dist,
  ...
  //  fmt::println("create a {} size bpm,replacer:{}", num_frames, k_dist);
...
auto BufferPoolManager::CheckedWritePage(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  ...
  if (!free_frames_.empty()) {  // 有空闲页
    ...
    disk_scheduler_->Schedule({false, frame->data_.data(), page_id, std::move(promise)});
  ...
  if (frame_id_opt.has_value()) {  // 有淘汰页
    auto frame_id = frame_id_opt.value();
    // 处理页框中之前的页
    auto frame = frames_[frame_id];
    auto old_page_id = frame->page_id_;
    //    fmt::println("Write replace {} to {}", old_page_id, page_id);
    // 旧页是脏页
    if (frame->is_dirty_) {
...
auto BufferPoolManager::CheckedReadPage(page_id_t page_id, AccessType access_type) -> std::optional<ReadPageGuard> {
  ...
  if (frame_id_opt.has_value()) {  // 有淘汰页
    ...
    auto frame = frames_[frame_id];
    auto old_page_id = frame->page_id_;
    //    fmt::println("Read replace {} to {}", old_page_id, page_id);
    // 旧页是脏页
    if (frame->is_dirty_) {
      FlushPage(old_page_id);
    }
...
auto BufferPoolManager::FlushPage(page_id_t page_id) -> bool {
  ...
  auto promise = disk_scheduler_->CreatePromise();
  auto future = promise.get_future();
  disk_scheduler_->Schedule({true, frame->data_.data(), page_id, std::move(promise)});
  future.get();
